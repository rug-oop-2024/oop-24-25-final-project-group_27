...
class Artifact(BaseModel):
    ...
    version: str = Field("1.0.0", description="Version of the artifact. Default is 1.0.0")
    tags: str = Field(None, description="Labels or keywords used to categorize or identify artifacts based on certain attributes")
    metadata: dict[str, str] = Field(None, description="A dictionary containing metadata for the artifact, such as experiment ID and run ID.")
    ...
    def __init__(self, **data):
        super().__init__(**data)
        self.id = self.generate_id()  # Generate the ID using asset_path and version
    ...
    def generate_id(self) -> str:
        ...
        sanitized_version = self.version.replace('.', '_').replace(';', '_').replace(',', '_').replace('=', '_')
